...
class Polygon:
    ...
    def __init__(self, point1: int, point2: int, point3: int, normal: Vec3, color: Vec3) -> None:
...
for z in [-3, 0]:
    ...
    bl = Vert(Vec3(x, z + 3, height))
    br = Vert(Vec3(x + 3, z + 3, height))
    new_verts = [tl, tr, bl, br]
    for vert in new_verts:
        verts.append(vert)
    tl = 0 + tri_num
    ...
    bl = 2 + tri_num
    br = 3 + tri_num
    polygon1 = Polygon(br, tr, tl, normalize(Vec3(0, 0, 1)), Vec3(25, 200, 40) * FV3(perlin_color[x + 10][z + 10]))
    polygon2 = Polygon(tl, bl, br, normalize(Vec3(0, 0, 1)), Vec3(25, 200, 40) * FV3(perlin_color[x + 10][z + 11]))
    polygons.append(polygon1)
    polygons.append(polygon2)
    tri_num += 4
...
while running:
    ...
    if held[" "] and grounded >= 0:
        F_app += Vec3(0, -movement_speed * 50, 0)
        grounded = -20
    if held["left"]:
    ...
    #"""
    Fnet = Vec3(0, 0, 0)
    F_f = Vec3(-velocity.x, 0, -velocity.z) * FV3(2.2)  # friction
    ...
    for mesh in polygons:
        for polygon in mesh:
            intersection = RayTriangle(verts[polygon.point1].point, verts[polygon.point2].point, verts[polygon.point3].point, ray_start, ray_end)
            ...
            if intersection is not False:# or on_surface:
